...
async def _engine_client_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(settings, "reports_dir", reports_path)
    ...
    return engine, session, client_model


@pytest.mark.asyncio
async def test_create_analysis_job_rejects_unsupported_mime(tmp_path, monkeypatch):
    engine, session, client_model = await _engine_client_session(tmp_path, monkeypatch)
    try:
...
def _report_reader() -> PydanticTechnicalReportReader:
    return PydanticTechnicalReportReader(LocalFileStorageAdapter())


@pytest.mark.asyncio
async def test_load_report_json_invalid_bytes(tmp_path, monkeypatch):
    engine, session, _ = await _engine_client_session(tmp_path, monkeypatch)
    try:
        bad_json_path = settings.reports_dir / "bad.json"
        bad_json_path.write_text("not valid json {", encoding="utf-8")
        with pytest.raises(ValidationError):
            load_report_for_api(_analyzed_job("bad.json"), report_reader=_report_reader())
    finally:
        await session.close()
        await engine.dispose()
...
async def test_load_report_json_missing_file(tmp_path, monkeypatch):
    ...
    try:
        with pytest.raises(FileNotFoundError):
            load_report_for_api(
                _analyzed_job("nope-missing.json"), report_reader=_report_reader()
            )
    finally:
        await session.close()
        await engine.dispose()
